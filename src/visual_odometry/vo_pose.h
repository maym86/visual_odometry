//
// Created by Michael May on 9/16/18.
//
...
static const size_t kMinPosePoints = 8;
static constexpr float kMax3DDist = 200;
...
void updatePose(const cv::Mat &K, VOFrame *last_frame, VOFrame *new_frame) {
    ...
    if (res > kMinPosePoints) {
        ...
        new_frame->points_3d = triangulate(last_frame->points, new_frame->points, K * cv::Mat::eye(3, 4, CV_64FC1),
